...
const roster = {
    ...
    teamGoscie: [
        ...
        'BONSON', 'BONUS RPK', 'BORIXON', 'BOSSKI ROMAN', 'CHIVAS', 'CIELOG', 'DEOBSON', 'DJ DECKS', 'DJ IKE', 'DIZKRET',
        'ELDO', 'EMAS', 'ERIPE', 'ERO', 'FAGATA', 'FISZ', 'FOKUS', 'GREEN', 'GSP', 'GURAL', 'GUZIOR',
        'KACZOR', 'KARA', 'KAZ', 'KEKE', 'KIZO', 'KOSI', 'KRZY KRZYSZTOF',
        'KUBAN', 'KUBANCZYK', 'KUKON', 'LAIKIKE1', 'LANEK', 'LECH ROCH PAWLAK', 'LJ KARWEL',
        'LONA', 'LOUIS V', 'MAGIERA', 'MALIK', 'MALPA', 'MATA', 'MATHEO', 'MEZO', 'MIUOSH', 'OLIWKA BRAZIL', 'OSTR',
        ...
        'WENA', 'WILKU', 'WINI', 'WLODI', 'WOJEK', 'YOUNG IGI', 'YOUNG MULTI',
        'ZABSON', 'ZEPPY ZEP', 'ZYTO', 'FILIPEK', 'NOON', 'FU', 'LIROY', 'KORAS', 'JURAS',
...
function createCharacterButton(name, container) {
    ...
    const handleSelection = (e) => {
        ...
        console.log(`[CharSelect] Tap on ${name} (Idx: ${charIndex}), Player: ${myIdx}, P1Conf: ${p1Confirmed}`);

        // --- ONLINE SELECTION LOGIC ---
        if (isOnline) {
            // ... (keep online logic as is)
        ...
        } else if (gameMode === '2V2_CHAOS') {
            // Support local 2v2 selection if reached
            if (!p3Confirmed) {
                if (p3Index === charIndex) { p3Confirmed = true; window.p3Selection = name; safeAudio('playSound', 'p1'); }
                else { p3Index = charIndex; safeAudio('playAttack'); }
            } else if (!p4Confirmed) {
    ...
    // REMOVED preventDefault touchstart to permit scrolling lists
    btn.addEventListener('touchstart', (e) => {
        // Just highlight on touchstart to feel responsive, but don't preventDefault to allow scroll
        const charIndex = allCharacters.indexOf(name)
        if (!p1Confirmed) p1Index = charIndex;
        else if (!p2Confirmed) p2Index = charIndex;
        updateMenuVisuals();
    }, { passive: true })
...
function updatePreviewWindow(charIndex, containerId, spriteId, nameId, unknownId) {
    ...
    if (preview && previewSprite && previewName) {
        ...
        if (data) {
            ...
            // Row 0, Frame 0 - pierwsza klatka, zmniejszona (150%) i przesunieta -70px
            previewSprite.style.backgroundSize = `${cols * 150}% ${rows * 100}%`
            previewSprite.style.backgroundPosition = '-30px 0%'
...
returnToMenu = function () {
    if (window.gameMode === 'TOURNAMENT') {
        const rounds = (window.currentTournamentConfig && window.currentTournamentConfig.rounds) || 10
        // VALIDATE: Only increment round if match was actually active and won
        if (window.matchOver && window.player && window.player.health > 0 && window.enemy && window.enemy.health <= 0) {
            // We won
...
function startGame() {
    ...
    if (p1ConfigToUse) {
        ...
        // Row 0, Frame 0 - pierwsza klatka
        vsP1Img.style.backgroundSize = `${cols * 100}% ${rows * 100}%`
        vsP1Img.style.backgroundPosition = '0% 0%'
    ...
    if (p2ConfigToUse) {
        ...
        // Row 0, Frame 0 - pierwsza klatka
        vsP2Img.style.backgroundSize = `${cols * 100}% ${rows * 100}%`
        vsP2Img.style.backgroundPosition = '0% 0%'
...
function triggerKey(key, type) {
    ...
    // Map WASD to Arrows for P2 (Guest) so local Phaser moves the right character
    if (role === 1) {
        if (finalKey === 'w') finalKey = 'arrowup';
        if (finalKey === 's') finalKey = 'arrowdown';
    ...
    // Polyfill properties for mobile browsers
    Object.defineProperty(event, 'keyCode', { get: () => data.keyCode || 0 });
    Object.defineProperty(event, 'which', { get: () => data.keyCode || 0 });
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (mobileToggle && mobileControls) {
        mobileToggle.addEventListener('click', () => {
            const currentDisplay = window.getComputedStyle(mobileControls).display;
            if (currentDisplay === 'none') {
                mobileControls.style.display = 'flex'
                mobileToggle.innerText = 'HIDE PAD'
            } else {
                mobileControls.style.display = 'none'
                mobileToggle.innerText = 'SHOW PAD'
            }
        })
    }
...
// 1. INPUT STATE SYSTEM (Prevents shared control issues)
window.onlineInputState = {
    P1: { up: false, left: false, down: false, right: false, punch: false, kick: false, taunt: false, super: false, special: false },
    P2: { up: false, left: false, down: false, right: false, punch: false, kick: false, taunt: false, super: false, special: false },
    P3: { up: false, left: false, down: false, right: false, punch: false, kick: false, taunt: false, super: false, special: false },
    P4: { up: false, left: false, down: false, right: false, punch: false, kick: false, taunt: false, super: false, special: false }
};

window.onlineInputPrevState = JSON.parse(JSON.stringify(window.onlineInputState));
window.onlineInputJustDown = JSON.parse(JSON.stringify(window.onlineInputState));

window.updateOnlineInputJustDown = function () {
    for (const tag in window.onlineInputState) {
        for (const action in window.onlineInputState[tag]) {
            const curr = window.onlineInputState[tag][action];
            const prev = window.onlineInputPrevState[tag][action];
            window.onlineInputJustDown[tag][action] = (curr && !prev);
            window.onlineInputPrevState[tag][action] = curr;
        }
    }
};
...
window.handleOnlineInput = function (data) {
    if (data.type === 'keydown' || data.type === 'keyup') {
        ...
        // Target is the person WHO SENT this. 
        // If I am Host (0), I receive from P2 (Guest).
        // If I am Guest (1), I receive from P1 (Host).
        const targetTag = (role === 0) ? 'P2' : 'P1';
        updateOnlineState(targetTag, key, isDown);
        return;
    }
    ...
    if (data.type === 'state_sync') {
        ...
        if (window.networkManager.playerIndex === 0) {
            ...
            if (payload.sender === 'P2') {
                ...
                if (Math.abs(curPos.x - payload.x) > 100) {
                    if (combat) { char.x = payload.x; char.y = payload.y; }
                    else { char.position.x = payload.x; char.position.y = payload.y; }
                } else {
                    if (combat) {
                        char.x += (payload.x - char.x) * 0.25;
                        char.y += (payload.y - char.y) * 0.25;
                    } else {
                        char.position.x += (payload.x - char.position.x) * 0.25;
                        char.position.y += (payload.y - char.position.y) * 0.25;
                    }
                }
                ...
                if (combat) {
                    ...
                    char.setData('energy', payload.energy || 0); // NEW: Sync Energy
                    char.setData('isAttacking', payload.isAttacking);
                    combat.syncUI();
                } else {
                    char.health = payload.health;
                    char.energy = payload.energy || 0;
...
function updateOnlineState(tag, key, isDown) {
    const s = window.onlineInputState[tag];
    if (!s) return;
    const k = key.toLowerCase();

    // Map keys to actions
    if (k === 'w' || k === 'arrowup') s.up = isDown;
    if (k === 's' || k === 'arrowdown') s.down = isDown;
    if (k === 'a' || k === 'arrowleft') s.left = isDown;
    if (k === 'd' || k === 'arrowright') s.right = isDown;
    if (k === ' ' || k === 'enter' || k === 'j') s.punch = isDown;
    if (k === 'c' || k === 'k') s.kick = isDown; // Removed 'o' AND 'v' (v might be RZUT/Throw for user)
    if (k === 'i') s.super = isDown;
    if (k === 'o') s.special = isDown; // 'o' is ONLY special now
    if (k === 'u') s.taunt = isDown;
}
...
window.addEventListener('keydown', (e) => {
    ...
    if (window.networkManager) {
        ...
        const myTag = (role === 0) ? 'P1' : (role === 1 ? 'P2' : (role === 2 ? 'P3' : 'P4'));

        updateOnlineState(myTag, e.key, true);
...
window.addEventListener('keyup', (e) => {
    ...
    if (window.networkManager) {
        ...
        updateOnlineState(myTag, e.key, false);
...
setInterval(() => {
    if ((gameMode === 'ONLINE' || gameMode === '2V2_CHAOS') && window.networkManager && window.networkManager.isOnline && gameStarted) {
        const combat = (window.phaserGame && window.phaserGame.scene) ? window.phaserGame.scene.getScene('CombatScene') : null;
        const isHost = window.networkManager.playerIndex === 0;
        const role = window.networkManager.playerIndex;

        let myChar = null;
        if (role === 0) myChar = combat ? combat.player : window.player;
        else if (role === 1) myChar = combat ? combat.enemy : window.enemy;
        else if (role === 2 && combat) myChar = combat.p3;
        else if (role === 3 && combat) myChar = combat.p4;

        if (!myChar) return;

        // Payload
        const payload = {
            sender: (role === 0) ? 'P1' : (role === 1 ? 'P2' : (role === 2 ? 'P3' : 'P4')),
            x: (myChar.x !== undefined) ? myChar.x : (myChar.position ? myChar.position.x : 0),
            y: (myChar.y !== undefined) ? myChar.y : (myChar.position ? myChar.position.y : 0),
            health: (myChar.getData) ? myChar.getData('health') : (myChar.health || 0),
            energy: (myChar.getData) ? myChar.getData('energy') : (myChar.energy || 0),
            isAttacking: (myChar.getData) ? myChar.getData('isAttacking') : (myChar.isAttacking || false)
        }
